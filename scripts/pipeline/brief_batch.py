...
def _batch_timeout_s():
    try:
        return max(60, int(os.environ.get("ASSETFRAME_BATCH_TIMEOUT_S", "3000")))
...
def _telemetry(message, model):
    """Per-request token + web-search usage, costed at BATCH (50%) prices. Best-effort: the ledger
    records the live usage numbers; this drives the manifest's est_cost_usd only."""
    ...
    cost = 0.5 * ((billable_in / 1e6) * bw.PRICE_IN_PER_MTOK
                  + (tout / 1e6) * bw.PRICE_OUT_PER_MTOK)      # 0.5 = batch discount
    return {"model": model, "input_tokens": tin + cr_in + cw_in, "output_tokens": tout,
...
def _run_batch(client, reqs, *, label, poll_interval, timeout):
    """Submit reqs = [(custom_id, params_dict)], poll until ended, return ({cid: message}, {cid: err}).
    Raises (submission/timeout) so the caller can fall back to the synchronous path. Per-request
    failures are returned in the errs map, never raised."""
    if not reqs:
        return {}, {}
    ...
    while True:
        ...
        if time.time() >= deadline:
            try:
                ...
                pass
            raise BatchTimeout(f"{label} batch {bid} did not finish within {timeout}s")
...
def _author_round(client, items, *, model, max_tokens, guidance_map, poll_interval, timeout):
    ...
    msgs, errs = _run_batch(client, reqs, label="author", poll_interval=poll_interval, timeout=timeout)
...
def author_briefs(items, *, model, max_tokens, poll_interval=None, timeout=None):
    ...
    timeout failure so run_daily can fall back to the synchronous writer."""
    ...
    # validation errors as guidance. (A batch request can't self-correct mid-flight.)
...
def review_briefs(items, *, model, max_tokens, poll_interval=None, timeout=None):
    ...
    for it in items:
        ...
        reqs.append((cid, {"model": model, "max_tokens": max_tokens, "system": system,
                           "messages": [{"role": "user", "content": user}]}))
    msgs, _errs = _run_batch(client, reqs, label="critic", poll_interval=poll_interval, timeout=timeout)
