...
class TestAppendOnlyLedger(unittest.TestCase):
    ...
    def _run(self, pred_path, ledger_path):
        ...
        return subprocess.run(
            [sys.executable, os.path.join(os.path.dirname(HERE), "scripts", "pipeline", "scoring", "score_report.py"), pred_path,
             "--hourly", self._hourly],
    ...
    def test_two_scores_append_not_rewrite(self):
        ...
        r1 = subprocess.run([sys.executable, os.path.join(os.path.dirname(HERE), "scripts", "pipeline", "scoring", "score_report.py"), p1,
                             "--hourly", self._hourly],
        ...
        p2 = self._make_predictions_file("AF-20250106-BBB", "2025-01-06 09:00",
                                         "2025-01-06 11:00", self._hourly)
        r2 = subprocess.run([sys.executable, os.path.join(os.path.dirname(HERE), "scripts", "pipeline", "scoring", "score_report.py"), p2,
                             "--hourly", self._hourly],
                            cwd=workdir, capture_output=True, text=True)
    ...
    def test_dry_run_writes_nothing(self):
        ...
        p1 = self._make_predictions_file("AF-20250106-AAA", "2025-01-06 09:00",
                                         "2025-01-06 11:00", self._hourly)
        r = subprocess.run([sys.executable, os.path.join(os.path.dirname(HERE), "scripts", "pipeline", "scoring", "score_report.py"), p1,
